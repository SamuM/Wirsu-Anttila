...
CREATE TABLE IF NOT EXISTS `wirsu-anttila`.`users` (
...
/* Tekee käyttäjästä adminin */

insert into `useradmins` (`user_id`, `admin_username`, `admin_email` )
select t2.`user_id`, t2.`user_name`, t2.`user_email`
from   `useradmins` t1
       inner join `users` t2 on t1.`user_id` = t2.`user_id`
where t2.`user_id` = 3;

INSERT INTO `wirsu-anttila`.`useradmins` (`adminID`, `user_id`, `admin_username`, `admin_email`) 
VALUES ('1', '1', 'Samu', 'samumukkala@gmail.com');

CREATE TABLE `wirsu_blog` (
  `postID` int(11) unsigned NOT NULL AUTO_INCREMENT,
  `postTitle` varchar(255) DEFAULT NULL, 
  `postCont` text,
  `postDate` datetime DEFAULT NULL,
...
INSERT INTO `wirsu_blog` (`postID`, `postTitle`, `postCont`, `postDate`)
VALUES
  (1,'Testidataa Lisäilen vain lololo','<h2>The öäöäöäöäääöÖÄÖÄÖ Mutants Are Revolting</h2>\r\n<p> äöäöä ÄÖÄÖÄÖWe don\'t have a brig. 
...
CREATE TABLE `pakinakuvat` (
  `pakinakuvatID` int(11) unsigned NOT NULL AUTO_INCREMENT,
  `kuvaID` int(11) unsigned NOT NULL,
  `postID` int(11) unsigned NOT NULL,
  PRIMARY KEY (`pakinakuvatID`),
  FOREIGN KEY (`kuvaID`) REFERENCES paavontekstit(`kuvaID`),
  FOREIGN KEY (`postID`) REFERENCES pakinat(`postID`) 
)ENGINE=InnoDB DEFAULT CHARSET=utf8;
